...
export function DeviceCard({ device, compact = false }) {
  ...
  return (
    <motion.article
      ...
      className={cn(
        'group relative flex flex-col justify-between cursor-pointer overflow-hidden rounded-cloud border border-white/40 bg-white/42 p-6 shadow-insetSoft backdrop-blur-xl transition-all duration-500 hover:bg-white/60 hover:border-white/60 hover:shadow-[0_12px_40px_rgba(31,41,55,0.08)]',
        device.status && 'shadow-[0_22px_58px_rgba(139,167,161,0.16)]',
    ...
    >
        ...
        <div
          className={cn(
            ...
            device.status
              ...
              : 'bg-white/40 text-slateSoft/60',
          )}
        >
          <Icon size={21} strokeWidth={1.8} />
        </div>
        <div
          className="origin-right scale-[0.85] opacity-90 transition-opacity duration-500 group-hover:opacity-100"
      ...
      <div className="relative z-10 mt-6 flex-grow">
        ...
        <p className="mt-1 text-[13px] text-graphite/70">{getDeviceMeta(device)}</p>
      </div>
      <div className={cn("relative z-10 mt-6 h-[5px] overflow-hidden rounded-full", device.status ? "bg-white/58" : "bg-white/30")}>
        {device.status ? (
          <motion.div
          ...
          <div
            className="h-full rounded-full bg-slateSoft/20 transition-all duration-500"
            style={{ width: progress }}
          />
        )}
      </div>
